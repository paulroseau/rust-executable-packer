...
- Supporting "exceptions" for a language refers to the ability to return a result in a function that can be "caught"/"handled" at some higher level (ie. some intermediate stack frame):
```
function a {
  try {
      x();
